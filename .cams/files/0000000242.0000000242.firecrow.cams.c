...
char *hex_from_id(int id){
  ...
  if(fexists(hex_path)){
    ...
    struct timespec t;
    timespec_get(&t, TIME_UTC);
    hex = dk_fmtmem("%s.%s.firecrow", hexstr((char *)&t.tv_nsec, sizeof(long)), hexstr((char *)&t.tv_sec, sizeof(long)));
    */
  }else{
...
struct commit *commit_init(int id){
  ...
  /*com->prior = ct_fread(path, HEXLEN);*/
  ...
  path = dk_fmtmem(".cams/%d/cindex", id);
  com->cindex = cindex_to_tree(path);
