...
static struct ct_tree *diff_list_staged(){
  ...
  struct ct_tree *from_files =  cindex_to_tree(get_current(), false);
...
static struct ct_tree *diff_list(char *from, char *to){
  struct ct_tree *from_files = cindex_to_tree(from, false);
  from_files->free = NULL;
  struct ct_tree *to_files = cindex_to_tree(to, false);
  to_files->free = NULL;
