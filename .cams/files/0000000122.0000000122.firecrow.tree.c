      ...
      *data = found_node->data;
      ...
      if(status == CT_FOUND){
        ...
        if(found_node->lt){
          ...
          if(!parent){
            printf("was no parent\n");
            parent = found_node->parent;
          }
          if(is_gt){ 
            parent->gt = found_node->lt;
          }else{ 
            parent->lt = found_node->lt;
          }
        }
        if(found_node->gt){
          status = _ct_find_place(
          ...
          if(is_gt){ 
            parent->gt = found_node->gt;
          }else{ 
            parent->lt = found_node->gt;
          }
        }
        return CT_MODIFIED;
      }else{
...
enum ct_status balance(struct ct_node *node){
