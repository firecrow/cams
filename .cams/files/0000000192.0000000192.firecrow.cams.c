...
static size_t HEXLEN = sizeof(long)*2;
...
int diff(int argc, char **argv){
  ...
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){ 
    ...
    if(file->prior){
        to_path = dk_fmtmem(".cams/files/%s", gen_path(file, false));
    }else{
...
void generate_cindex(int16_t id){
  ...
  struct ct_tree *files = flist(id);
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){
    kv.data = ((struct ent *)kv.data)->hex;
...
void show_files(int id){
  char *hex = hex_from_id(id);
  char *chex;
  char *path_to = dk_fmtmem(".cams/%d/cindex", id);
  struct ct_key_data kv = {NULL, NULL};
  struct ct_tree *files =  cindex_to_tree(path_to);
  bool first = true;
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){
    chex = (char *)kv.data; 
    if(!strncmp(chex, hex, 1024)){
      if(!first)
        printf(", ");
      first = false;
      printf("%s", kv.key);
