...
int add(int argc, char **argv){
  ...
  for(i = 0; i<len; i++){
    file = ent_init(dk_fmtmem(list[i]), NULL, NULL);
    if(snprintf(cmd, 1024, "cp -v %s .cams/files/%s", file->path, file->spath, true) > 1023){
      fprintf(stderr, "file too long for buffer\n");
    }
    /* system(cmd); */
    printf("system cmd commented out\n");
  }
  return 0;
}
...
int unadd(int argc, char **argv){
  ...
  argv += 2;
  while(--argc >= 0){
    file = ent_init(dk_fmtmem(argv[argc]), NULL, NULL);
...
int diff(int argc, char **argv){
  ...
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){ 
    ...
    if(!file->hex){
      fname = dk_fmtmem(".cams/files/%s", gen_path(file));
    }else{
      fname = dk_fmtmem("/dev/null");
    }
    cmd = dk_fmtmem("diff %s .cams/files/%s", fname, gen_path(file));
    printf("[%s] %d..%d\n", file->path);
    fflush(stdout);
    system(cmd);
    printf("\n");
...
void checkout(int16_t from){
  ...
  while(f){
    printf("%s: %d\n", f->path, f->prior);
    cmd = dk_fmtmem("cp -v .cams/%d/files/%s %s", f->bid, f->path, f->path);
    system(cmd);
    dk_free(cmd);
  }
  */
...
char *find_prior(int16_t id, struct ent *file){
  ...
  int16_t x = 0;
  int16_t p = 0;
  b = dk_fmtmem(".cams/index/%s", file->spath);
  idx_file = fopen(b, "r");
  ...
  return p;
  */
  return dk_fmtmem("may not need this");
}
...
struct ent *ent_init(char *path, char *spath, char *hex){
  struct ent *cur = dk_malloc(sizeof(struct ent));
  cur->path = path;
  cur->spath = san_fname(path, true);
  cur->hex = NULL;
  return cur;
}

struct ent *ent_from_line(char *line){
  int hexlen = sizeof(long)*2;
  char *hex = dk_malloc(hexlen+1);
  memcpy(hex, line, hexlen); 
  hex[hexlen] = '\0';
  line += hexlen+1;/* ahead of . */
  int pathlen = strlen(line);
  char *path= dk_malloc(sizeof(char)*pathlen+1);
  memcpy(path, line, pathlen); 
  path[pathlen] = '\0';
  return ent_init(path, san_fname(path, true), hex);
}


char *gen_path(struct ent *file){
  return dk_fmtmem("%s.%s", file->hex, file->spath);
}
...
struct ct_tree *flist(int id){
  ...
  while((dp = readdir(d)) != NULL){
    if(!strncmp(".", dp->d_name, 1) || !strncmp("..", dp->d_name, 2))
      continue;
    cur = ent_init(NULL, dk_fmtmem(dp->d_name), NULL);
    kv.key = cur->path;
    kv.data = cur;
    ct_tree_set(tree, &kv);
  }
...
struct ct_tree *diff_list(int from, int to){
  ...
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){
    ...
    file = (struct ent *)kv.data;
    file->prior = find_prior(from+1, file);
  }
  return files;
}
