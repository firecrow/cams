...
int ct_subp(struct ct_subp *subp, int flags){
  int ins[2];
  int outs[2];
  int errs[2];

  if(flags | CT_USE_STDIN){
    pipe(ins);
    subp->stdin = fdopen(ins[1], "w");
  }
  if(flags | CT_USE_STDOUT){
    pipe(outs);
    subp->stdout = fdopen(outs[0], "r");
  }
  if(flags | CT_USE_STDERR){
    pipe(errs);
    subp->stderr = fdopen(errs[0], "r");
  }
  ...
  if(pid == -1){
    ...
    if(flags | CT_USE_STDOUT) close(ins[0]);
    if(flags | CT_USE_STDOUT) close(outs[1]);
    if(flags | CT_USE_STDERR) close(errs[1]);
    return WEXITSTATUS(status);
  }else{
    if(flags | CT_USE_STDIN) pipe(ins);
    if(flags | CT_USE_STDOUT) dup2(ins[0], 0);
    if(flags | CT_USE_STDOUT) dup2(outs[1], 1);
    if(flags | CT_USE_STDERR) dup2(errs[1], 2);
