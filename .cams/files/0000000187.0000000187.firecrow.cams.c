...
char *gen_path(struct ent* cur){
  return dk_fmtmem("%s.%s", cur->hex, cur->spath);
}
...
int commit(int argc, char **argv){
  ...
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){
    ...
    cmd = dk_fmtmem("mv .cams/%d/files/%s .cams/files/%s", id, file->spath, gen_path(file));
    fprintf(stderr, "2\n");
    fprintf(stderr, "%s\n", cmd);
    dk_free(cmd);
    */
...
int diff(int argc, char **argv){
  ...
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){ 
    ...
    if(file->tid){
        to_path = dk_fmtmem(".cams/%d/files/%s", file->tid, file->spath);
    }else{
        to_path = dk_fmtmem("/dev/null");
    }
    if(file->bid){
        from_path = dk_fmtmem(".cams/%d/files/%s", file->bid, file->spath);
    }else{
        from_path = dk_fmtmem("/dev/null");
    }

    cmd = dk_fmtmem("diff %s %s", from_path, to_path);
    printf("[%s] %d..%d\n", file->path, file->bid, file->tid);
    fflush(stdout);
    system(cmd);
    printf("\n");
...
struct ct_tree *flist(int id){
  ...
  while((dp = readdir(d)) != NULL){
    ...
    kv.key = cur->path;
    kv.data = cur;
    enum ct_status status = ct_tree_set(tree, &kv);
  }
  return tree;
}
...
struct ct_tree *diff_list(int from, int to){
  ...
  if(xres->a->len){
    while(ct_tree_next(xres->a, &kv) != CT_NOT_FOUND){
      cur = ent_init(kv.key, NULL); 
      cur ->tid = *(int16_t *)kv.data; 
      kv.data = cur;
