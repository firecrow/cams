...
void generate_cindex(char *cid){
  ...
  if(com->parent){
    ctree = cindex_to_tree(com->parent, false);
  }else{
    ctree = ent_tree_init();
  }
...
struct ct_tree *cfiles(char *cid){
  struct ct_tree *tree = cindex_to_tree(cid, false);
...
struct ct_tree *cindex_to_tree(char *cid, bool time){
  struct ct_tree *tree = ent_tree_init();
  if(!cid){
    return tree;
  }
  ...
  while(fgets(buff, 1024, file) != NULL){
    ...
    if(time){
      ckv.key = cur->cid;
      if(ct_tree_get(coms, &ckv)){
        ckv.data = commit_init(ckv.key);
        ct_tree_set(coms, &ckv);
      }
      com = ckv.data;
      cur->time.tv_sec = com->time.tv_sec;
      cur->time.tv_nsec = com->time.tv_nsec;
    }
    kv.data = cur;
    ct_tree_set(tree, &kv);
  }
...
struct commit *commit_init(char *cid){
  ...
  com->name = pieces[2];
  com->time.tv_sec = atoi(pieces[1]);
  com->time.tv_nsec = atoi(pieces[0]);
...
int show_commit(int argc, char **argv){
  ...
  fprintf(stderr, "all:\n");
  kv.key = NULL;
  while(!ct_tree_next(cindex_to_tree(com->cid, false), &kv)){
    struct ent *cur = (struct ent *)kv.data;
    fprintf(stderr, "  %s:%s\n", kv.key, cur->cid);
  }
...
int add(int argc, char **argv){
  ...
  for(i = 0; i<len; i++){
    ...
    if(snprintf(cmd, 1024, "cp -v %s .cams/stage/files/%s", file->path, file->spath) > 1023){
...
int commit(int argc, char **argv){
  ...
  while(!ct_tree_next(files, &kv)){
    ...
    cmd = dk_fmtmem("mv .cams/stage/files/%s .cams/files/%s", file->spath, file_path);
    dk_free(file_path);
    if(system(cmd)){
