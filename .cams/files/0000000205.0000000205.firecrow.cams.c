...
int diff(int argc, char **argv){
  ...
  struct ent *file;
  if(argc == 2){
    from = next_commit_id()-1;
    fprintf(stderr, "finish:%d\n", from);
    files = diff_list(from, -1);
  }else if(argc == 4){
    from = atoi(argv[2]);
    to = atoi(argv[3]);
...
struct ct_tree *diff_list(int from, int to){
  ...
  struct ct_tree *from_files;
  struct ct_tree *to_files;
  if(to == -1){
    to_files = flist(next_commit_id());
  }else{
    to_files = cindex_to_tree(path_to);
  }
  from_files =  cindex_to_tree(path_from);
  dk_free(path_from);
  dk_free(path_to);
