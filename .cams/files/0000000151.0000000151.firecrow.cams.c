...
int diff(int argc, char **argv){
  ...
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){ 
    ...
    cmd = dk_fmtmem("diff %s .cams/%d/files/%s", fname, file->tid, file->name);
...
char *san_fname(char *fname, bool unsan){
...
struct ct_tree *diff_list(int from, int to){
  ...
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){
    ...
    file = (struct ent *)kv.data;
    file->bid = find_prior(from+1, file->name);
  }
  return files;
}
