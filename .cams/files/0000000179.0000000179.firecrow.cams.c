...
struct ct_tree *cindex_to_tree(FILE *file){
  struct ct_tree *tree = ct_tree_alpha_init();
  return tree;
}
...
struct ct_tree *diff_list(int from, int to){
  ...
  FILE *fromf = dk_open(path_from, "r");
  dk_free(path_from);

  char *to_from = dk_fmtmem(".cams/%d/cindex", to);
  FILE *tof = dk_open(path_from, "r");
  dk_free(path_from);

  struct ct_tree *from_files =  cindex_to_tree(fromf);
  struct ct_tree *to_files = cindex_to_tree(tof);
  struct ct_tree *files = ct_tree_alpha_init();
  fclose(fromf);
  fclose(tof);

  /* generate intersection inverse */
