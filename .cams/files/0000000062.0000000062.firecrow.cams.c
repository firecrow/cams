...
int head_id(){
  FILE *rdf;
  rdf = fopen(".cams/head", "r");
}

struct ent *read_cindex(FILE *in){
  ;
}

void write_cindex(FILE *out, struct ent *files){
  ;
}

void build_cindex(FILE *out, uint16_t id){
  /* get list of files incommit (flist) */;
  /* get index from previous commit (read_cindex) */
  /* merge em */
  /* write it out */
}
