...
void ct_free_ent_node(struct ct_node *node){
  struct ent *cur = (struct ent *)node->data;
  ent_free(cur);
  dk_free(node);
}
...
int push(int argc, char **argv){
  ...
  if(nid < rlatest){
    ...
    i = rlatest;
    while(++i < nid){
      kv.key = dk_fmtmem("%d", i);
      ct_tree_set(files, &kv);
      gather_files_for_push(files, i);
    }
...
void generate_cindex(int16_t id){
  ...
  if(id > 1){
    ...
    fprintf(stderr, "<-> cindex %d <->\n", dk_count()); 
    ctree = ent_tree_init();
    fprintf(stderr, "<-> cindex out %d <->\n", dk_count()); 
  }

  struct ct_key_data kv = {NULL, NULL};
  fprintf(stderr, "<-> files %d <->\n", dk_count()); 
  struct ct_tree *files = flist(id);
  fprintf(stderr, "<-> files out %d <->\n", dk_count()); 
  fprintf(stderr, "<-> ctree %d <->\n", dk_count()); 
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){
    cur = (struct ent *)kv.data;
    fprintf(stderr, "<-> dupstr %d <->\n", dk_count()); 
    cur->hex = dupstr(hex);
    fprintf(stderr, "<-> dupstr out %d <->\n", dk_count()); 
    fprintf(stderr, "<-> set %d <->\n", dk_count()); 
    ct_tree_set(ctree, &kv); 
    fprintf(stderr, "<-> set out %d <->\n", dk_count()); 
  }
  fprintf(stderr, "<-> ctree out %d <->\n", dk_count()); 

  /* line ------------------------- */
  fprintf(stderr, "%d,%d\n", files->len, ctree->len); 
  files->free = ct_free_node_only;
  fprintf(stderr, "(x) files %d\n", dk_count());
  ct_tree_free(files);
  fprintf(stderr, "(<) files %d\n", dk_count());
  fprintf(stderr, "(x) ctree %d\n", dk_count());
  ct_tree_free(ctree);
  fprintf(stderr, "(<) ctree %d\n", dk_count());
  dk_free(hex); 
  fprintf(stderr, "<-> %d <->\n", dk_count()); 
...
int main(int argc, char **argv){
  ...
  if(ret != -1){
    if(count = dk_count()){
      fprintf(stderr, "error memory count not zero for %s: %d\n", argv[1], count);
      /*return 123;*/
    }
    return ret;
  }
