...
struct ct_tree *cindex_to_tree(char *path){
  ...
  while(fgets(buff, 1024, file) != 0){
    cid = dk_malloc(sizeof(int16_t));
    memcpy(cid, buff, sizeof(int16_t));
    ptr = buff+sizeof(int16_t);
    len = strlen(ptr); 
    ptr += len-1;
    if(*ptr== '\n'){
      *ptr = '\0';
    ...
    fname = dk_fmtmem("%s", buff+sizeof(int16_t));
    kv.key = fname;
    kv.data = cid;
    ct_tree_set(tree, &kv);
  }
...
struct ct_tree *diff_list(int from, int to){
  ...
  printf("before\n");
  xres = ct_tree_intersection(from_files, to_files);
  if(xres->a->len){
    printf("a:%d\n", xres->a->len);
