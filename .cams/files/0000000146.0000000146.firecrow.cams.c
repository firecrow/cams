...
int rm(int argc, char **argv){
  ...
  while(--argc >= 0){
    ...
    sfname = sanatize(fname);
    sfname = dk_fmtmem("%s\n", sfname);
    fwrite(sfname, 1, strlen(sfname), file);
    char *fpath = dk_fmtmem(".cams/%d/files/%s", id, fname);
    unlink(fpath);
  }
...
struct ct_tree *ftree(int id){
  ...
  while((dp = readdir(d)) != NULL){
    if(!strncmp(".", dp->d_name, 1) || !strncmp("..", dp->d_name, 2))
      continue;
    cur = ent_init(dk_fmtmem(dp->d_name));
    kv.key = cur->name;
    kv.data = cur;
    ct_tree_set(tree, &kv);
  }
