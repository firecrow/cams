...
char *gen_cid(char *name){
  ...
  char *cid = dk_fmtmem("%010d.%010d.%s", count, count, "testuser");
  return cid;
}
...
char *gen_cid(char *name){
  ...
  char *cid = dk_fmtmem("%010d.%010d.%s", t.tv_nsec, t.tv_sec, name);
  return cid;
}
...
struct commit *commit_init(char *cid){
  ...
  if(ct_split(cid, '.', 0, &pieces) != 3){
    fprintf(stderr, "invalid id, split by . did not result in 3 pieces\n");
    exit(123);
  }
  com->millis = pieces[0];
  com->epoch = pieces[1];
...
int com_before(struct commit *a, struct commit *b){
  int time_a = (atoi(a->epoch) * 1000000000)+atoi(a->millis); 
  int time_b = (atoi(b->epoch) * 1000000000)+atoi(b->millis);
