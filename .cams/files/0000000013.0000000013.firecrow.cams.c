...
int fmtmem(char *dest, size_t len, char *fmt, args...){
  va_list va; 
  va_start(&va, args);
  if(vfnprintf(desv, len, fmt, va) > len-1){
    fprintf(stderr, "fmtmem too long\n");
    return -1;
  };
  return 0;
}

int fprev(char *dest, int id, char *fname){
} 
...
struct ent *flist_commit(int id){
   d = dirent(".cams/%d/files", id);
   struct ent *prev = NULL;
   for(f in d){
      ent = malloc(sizeof(ent));
      ent->name = strcpy(d->f_name);
      if(prev){
        prev->next = ent;
      }
      prev = ent;
