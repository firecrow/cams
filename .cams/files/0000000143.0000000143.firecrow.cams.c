...
void generate_cindex(int16_t id){
  ...
  if(id > 1){
    ...
    while(true){
      ...
      rmfilesp = rmfiles;
      while(rmfilesp){
        if(!strcmp(prev_buff, rmfilesp->name))
          break;
        rmfilesp = rmfilesp->next;
      }
      if(filesp == NULL && rmfilesp == NULL){
...
struct ent *rmlist(int id){
  ...
  if(access(fname, F_OK) != -1){
    ...
    while(fgets(line, 1023, file) != NULL){
      ...
      cur = dk_malloc(sizeof(struct ent));
      cur->name = dk_fmtmem(line);
      cur->next = head;
      head = cur;
    }
  }
