...
int add(int argc, char **argv){
  ...
  for(i = 0; i<len; i++){
    file = ent_init(dk_fmtmem(list[i]));
    if(snprintf(cmd, 1024, "cp -v %s .cams/%d/files/%s", file->name, id, file->sname) > 1023){
...
char *sum_fname(char *fname, bool san){
  int len = strlen(fname);
  char escape;
  char replace;
  char *n_name = dk_malloc(sizeof(char)*len+1);
  char *np = n_name;
  if(san){
    escape = '/';
    replace = 26;
