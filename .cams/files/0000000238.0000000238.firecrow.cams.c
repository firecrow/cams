...
struct ct_tree *files_to_tree(char *base, char **paths){
  char *path;
  char *body;
  FILE *file;
  int len;
  ...
  while(paths != NULL){
    path = dk_fmtmem("%s/%s", base, *paths);
    if(fexists(path)){
      len = flen(path);
      body = dk_malloc(len+1);
      file = dk_open(path, "r");
...
struct commit *commit_init(int id){
  ...
  struct ct_key_data kv = {NULL, NULL};
  char *file_names[] = {"hex", "prior", "message", "time", NULL};
  struct ct_tree *meta = files_to_tree(basedir,file_names);
  kv.key = "hex";
  if(ct_tree_get(meta, &kv) != CT_NOT_FOUND){
     com->hex = kv.data;
  }
  kv.key = "prior";
  if(ct_tree_get(meta, &kv) != CT_NOT_FOUND){
