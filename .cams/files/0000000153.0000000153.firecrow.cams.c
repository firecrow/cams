...
int diff(int argc, char **argv){
  ...
  while(ct_tree_next(files, &kv) != CT_NOT_FOUND){ 
    ...
    cmd = dk_fmtmem("diff %s .cams/%d/files/%s", file->name, file->tid, file->sname);
...
char *san_fname(char *fname, bool san){
  ...
  char escape;
  char replace;
  char *n_name = dk_malloc(sizeof(char)*len);
  char *np = n_name;
  if(san){
    escape = '/';
    replace = 26;
