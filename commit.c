...
void generate_cindex(char *cid){
  ...
  char *rmindex_path = dk_fmtmem(".cams/%s/rmindex", cid);
  FILE *rmindexf = dk_open(rmindex_path, "w+");

  kv.key = NULL;
  while(ct_tree_next(rmfiles, &kv) != CT_NOT_FOUND){
    fprintf(rmindexf, "%s:%s\n", ((struct ent *)kv.data)->cid, kv.key);
  }
...
/*
void generate_rmindex(char *cid){
  struct ct_tree = previous rmindex file
  struct ct_tree *files = flist();
  remove flist files from removes
  generate rmindex file 
}
*/
...
char *gen_cid(char *name){
  ...
  if((d = opendir(".cams")) == NULL){
    fprintf(stderr, "unable to open dir: %s", ".cams");
    exit(123);
  }
  int count = 1;
...
char *gen_cid(char *name){
  struct timespec t;
  clock_gettime(CLOCK_REALTIME_PRECISE, &t);
...
struct ct_tree *rmlist(char *cid){
  ...
  if(access(fname, F_OK) != -1){
    ...
    while(fgets(line, 1023, file) != NULL){
      ...
      kv.key = dupstr(line);
      kv.data = ent_init(dupstr(line), NULL);
      ct_tree_set(tree, &kv);
    }
  }
  dk_free(fname);
...
char *lookup_or_die(char *millis){
  ...
  char *conflicted = dk_fmtmem(".cams/millis/%s.conflicts", millis);
  if(fexists(expected)){
    return ct_fread(expected);
  }else if(fexists(conflicted)){
    char *conflicts = ct_fread(conflicted);
